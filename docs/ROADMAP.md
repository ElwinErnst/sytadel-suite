...
- **🟡 Migraciones controladas** — `DB_SYNC=true` es red flag si un AppSec lee la config. Migrar a TypeORM migrations por servicio. **auth-api ✅**: `DB_SYNC=false` por default + baseline `Init` (12 entidades) con `migrationsRun` on boot; validado por el Smoke CI en DB fresca (PR auth-api #5 + bump #12). De paso se sacó un `console.log` que filtraba la password de la DB. Pendiente: **billing-api** (código ya defaultea false pero el compose fuerza `DB_SYNC="true"` y falta baseline); vault-api usa SQL init scripts; zerotrust-api no usa TypeORM synchronize
