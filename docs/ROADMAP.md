...
- **🟡 Firma asimétrica ZT (gateway→Vault)** — el HMAC compartido en el hop gateway→Vault se reemplaza por **Ed25519 asimétrico**: el gateway firma con una clave privada que solo él tiene (`ZT_SIGNING_PRIVATE_KEY`), Vault verifica con la pública vía keyring `ZT_VERIFY_PUBLIC_KEYS` = `{kid: pem}` (rotación sin downtime; un secreto compartido de menos). Protocolo **v2**: headers `x-zt-v:2` / `x-zt-alg:ed25519` / `x-zt-kid` + firma en `x-zt-sig`; el string canónico ata `alg`+`kid` arriba (anti-downgrade/kid-swap). **Migración dual-mode y reversible:** Vault despacha por `x-zt-v` (1→HMAC, 2→Ed25519) con `ZT_ACCEPT_V1_HMAC` (default `true`); el gateway elige con `ZT_SIGN_MODE=hmac|ed25519`. Las 3 slices están **mergeadas** (vault #17, zerotrust #4, compose #23; bump de submódulos = meta #24) pero **ships DORMANT: el default sigue siendo HMAC**. Encender = `ZT_SIGN_MODE=ed25519` + proveer el par de claves (mismo kid en los dos lados); rollback = volver a `hmac`, sin redeploy de Vault. Retirar HMAC después con `ZT_ACCEPT_V1_HMAC=false`. **Gap cerrado de paso:** Vault ahora recomputa `sha256(body)` vs `x-zt-body-sha256` (antes confiaba en el header a ciegas); caveat residual documentado: uploads multipart no exponen raw body, ahí el hash del body aún no queda atado
...
- **HMAC compartido** entre ZT y Vault — **mitigación entregada, no cerrada**: la firma asimétrica Ed25519 (v2) ya está mergeada y verificable dual-mode, pero el gateway todavía firma HMAC por default (ships dormant, ver hardening track). El riesgo sigue activo hasta el flip a `ZT_SIGN_MODE=ed25519` y el retiro de v1. Alcance: solo el hop gateway→Vault; policies/notary/audit siguen en HMAC
